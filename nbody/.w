...
def init_cond(n, b = 1.6):
    ...
    if n == 1:
        ...
        r_venus = np.array([6.611520265276862E-01, 2.935831802357576E-01, -3.442573108553538E-02])
        ...
        N = 3
        m_sun, m_earth, m_orb = 1.989e30, 5.972e24, 1e14
        d_e = 149.6e9
        l1 = opt.root_scalar(lambda r: get_l1(r, m_sun, m_earth, d_e), bracket=(.1, d_e-.1))
        d_orb = d_e - l1.root
        T_earth = 365 * 24 * 3600
        T_orb = T_earth
        ...
        x_sun = np.zeros(3)
        v_sun = np.zeros(3)
        x_orb = np.array([d_e/2, d_e*np.sqrt(3)/2*(1+4e-3), 0])
...
ti, tf = 0, 8
print(r0)
sol = integ.solve_ivp(lambda t, r: g(t, r, m, N), [ti, tf], r0, max_step=.01)
print(sol.status)
print(sol.message)

#def abs_v(sol, t):
#    return sol.y[15, t]**2 + sol.y[16, t]**2
#print(abs_v(sol, 10) /abs_v(sol, 0))

#for t in range(0, sol.t.size, 200):
#    for i in range(N):
#        plt.plot(sol.y[i*3], sol.y[i*3+1])
#    #plt.xlim(-10, 10)
#    #plt.ylim(-10, 10)
#    plt.show()
...
#plt.xlim(-10, 10)
#plt.ylim(-10, 10)
#plt.savefig("../Article/figs/sun_earth_mars_orbs.pdf", dpi=1000, transparent=True, bbox_inches="tight")
plt.show()

#for i in range(N):
#    plt.plot(sol.t, sol.y[8], label=i)
#plt.legend()
#plt.xlim(-10, 10)
#plt.ylim(-10, 10)
#plt.show()
...
def get_V(r, m, N):
    ...
    dist = np.sum(d**2, axis = -1)
    np.fill_diagonal(dist, 1)
    V = - m[:, _] * m[_, :] / np.sqrt(dist)
    np.fill_diagonal(V, 0)
    return V.sum()


#sol1 = integ.solve_ivp(lambda t, r: g(t, r, m, N), [ti, tf], r0, max_step=.1)
...
#plt.plot(sol1.t, E1, label="Maximum step: 0.1")
...
plt.legend()
#plt.savefig("../Article/figs/sun_earth_mars_energy.pdf", dpi=1000, transparent=True, bbox_inches="tight")
plt.show()
#plt.plot(sol.t, V)
